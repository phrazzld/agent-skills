...
1. **Investigate** — Problem space, user impact, prior art (parallel agents)
   ...
   **GATE: All web research MUST use the `/research` skill (Skill tool). Do NOT substitute raw WebSearch agents — `/research` routes through Exa for code context.**

   | Agent | Focus |
   |-------|-------|
   | Problem explorer | What's the real problem? Who has it? How painful? |
   | **`/research web-search`** | Reference implementations, prior art, best practices (uses Exa code context) |
...
1. **Absorb** — Read locked product spec, investigate codebase, research patterns.
   ...
   **GATE: All web research MUST use the `/research` skill (Skill tool). Do NOT substitute raw WebSearch agents.**

   | Agent | Focus |
   |-------|-------|
   | Codebase explorer | Existing patterns, touch points, docs/context + CODEBASE_MAP context |
   | **`/research web-search`** | Reference implementations, best practices, prior art (uses Exa code context) |
